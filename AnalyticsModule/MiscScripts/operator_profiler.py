...
def train_profiler_model(data_path='synthetic_machine_data_v3.csv'):
    """
    Loads machine data, engineers performance features for each operator,
    and uses KMeans clustering to group them into performance tiers.
    """
    ...
    # We need to create a "profile" for each operator.
    # We'll group the data by Operator_ID and calculate their average performance.
    print("2. Engineering operator performance profiles...")

    # Calculate total engine hours for each operator to normalize idling time
    ...
    # We use .replace to avoid division by zero if an operator has 0 cycles/hours
    ...
    # We create a pipeline to first scale the data, then apply KMeans.
    # This is crucial because features are on different scales.
    print("\n3. Building the K-Means clustering pipeline...")

    # We will find 3 clusters: e.g., Top, Average, Needs Improvement
    N_CLUSTERS = 3
    ...
    # The cluster labels (0, 1, 2) are our new "performance tiers"
    operator_profiles['performance_cluster'] = pipeline.predict(X)
    
    model_filename = 'operator_profiler_model.joblib'
    joblib.dump(pipeline, model_filename)
    print(f"\n5. Profiler model saved successfully as '{model_filename}'")
    ...
    print("\nNext step: Interpret what each cluster means by analyzing their average stats.")


if __name__ == "__main__":
    train_profiler_model()
